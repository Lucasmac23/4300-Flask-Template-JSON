...
def executeQuerySearch(personalValues_query, personalExperience_query, personalValuesWeighting, personalSkillsWeighting, num_return_industries=5, num_return_companies_per_industry=3, ):
    try:
        ...
        temp = 0 
        scoresReturn = []
        while len(scoresReturn)<num_return_industries:
            industry, _, _ = combinedScores[temp]
            if industry not in industry_reviews:
                reviews = get_random_reviews(tokenized_df, industry)
                industry_reviews[industry] = reviews
                scoresReturn.append(combinedScores[temp])
            temp = temp + 1
    ...
    # top_companies is of type (company, description, simScore)[]
        return {'scores': scoresReturn, 'reviews': industry_reviews}
